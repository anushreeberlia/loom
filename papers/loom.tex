...
a mean outfit score of 0.218 with a 6.0\% hard violation rate, compared to
0.125 score and 14.0\% violations for a category-constrained random
baseline---a 74\% improvement in score and 57\% reduction in violations.
Removing direction reranking drops score by 90\%; removing blended
embeddings increases violations by 67\%.  The system generates three
stylistically distinct outfits in under 5~seconds on commodity hardware.
...
CLIP's text encoder places material-related terms in a space where
physical properties (weight, warmth, drape) create meaningful geometric
structure.  The heavy/light context vectors act as learned ``probes'' into
this structure, extracting material weight without an explicit taxonomy.
...
choices to overall outfit coherence.
...
per configuration).  All rows share the same anchor set.  Random baseline
uses category-constrained random retrieval instead of ANN embedding search
(separate 50-anchor run; full system scored 0.232 / 4.7\% violations in that
run, consistent with the 0.218 baseline here given stochastic retrieval).}
\label{tab:ablation}
\begin{tabular}{lccc}
\toprule
...
$-$ Blended embed        & 0.163 & $-$0.055 & 10.0 \\
...
\midrule
Random retrieval\textsuperscript{$\dagger$} & 0.125 & $-$0.093 & 14.0 \\
\bottomrule
\end{tabular}
\end{table}
...
slots but ignores embedding similarity---scores 0.125 with 14.0\% hard
violations.  The full system's 74\% improvement over random confirms that
embedding-based retrieval provides substantial value beyond category-level
slot filling.

Direction reranking is the single most impactful individual component,
accounting for 90\% of the total score ($-$0.196).  Without it, outfits
are stylistically indistinct across the three directions.

Distance noise---the multiplicative perturbation applied during
retrieval---has the second-largest impact ($-$0.073).  Removing it makes
retrieval deterministic, which paradoxically \emph{increases} violation rate
to 12\%: the same top candidates are retrieved repeatedly, leading to
redundant outfit compositions with less room for the scoring function to
find compatible combinations.

Blended embeddings and occasion filtering contribute similar improvements
($\sim$0.05), confirming that text metadata and semantic occasion priors
each provide information not captured by image similarity alone.

Formality checking has the smallest marginal contribution ($-$0.013),
suggesting that the other constraints already handle most formality clashes
implicitly.
...
To validate that automated scores correlate with subjective outfit quality,
we conduct a single-annotator study on 30 randomly sampled anchors.  For
each anchor, we present the full-system outfit, the random-retrieval
baseline outfit, and a no-direction ablation outfit (stylistically
indistinct) side-by-side without labels.  The annotator rates each on a
1--5 Likert scale for coherence, occasion-appropriateness, and wearability.

\begin{table}[h]
\centering
\small
\caption{Human evaluation (N=30 anchors, single annotator).  Mean Likert
scores (1--5) and win rate vs.\ random baseline.  \textit{To be filled after
running \texttt{eval/human\_eval\_export.py} and completing annotation.}}
\label{tab:human}
\begin{tabular}{lcccr}
\toprule
System & Coherence & Occasion & Wear & Win vs Random \\
\midrule
Full system & --- & --- & --- & --- \\
No-direction & --- & --- & --- & --- \\
Random       & --- & --- & --- & --- \\
...
scoring models at larger scale.

\paragraph{Occasion prior coverage.}
Extending to new occasions requires only writing new vibe/anti-vibe
strings---no retraining.  Quality depends on the prose descriptions capturing
...
on a 620-item catalog show that the full system achieves a 74\%
improvement in outfit score over a category-constrained random baseline,
with each component contributing measurably.  The
system generates three stylistically distinct outfits in under 5 seconds on
